...
void LocalState::add_connection(uuid uuid_a, uuid uuid_b,
    ...
    this->connections[uuid_a].insert_or_assign(uuid_b, std::make_shared<Connection>(con));
    this->connections[uuid_b].insert_or_assign(uuid_a, std::make_shared<Connection>(con));
}
